...
public final class ConsumeCondition extends AbstractCondition {
    ...
    public Text getDescription() {
        Text item = itemId.isBlank()
                ? ModI18n.text("condition.lumenechallenge.consume.any_item")
                : ModI18n.text(Registries.ITEM.get(Identifier.of(itemId)).getTranslationKey());
        return switch (mode) {
            case EAT -> ModI18n.text("condition.lumenechallenge.eating", item);
            case DRINK -> ModI18n.text("condition.lumenechallenge.drinking", item);
        };
    }
