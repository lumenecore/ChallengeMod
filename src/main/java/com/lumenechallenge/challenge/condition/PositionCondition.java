...
public final class PositionCondition extends AbstractCondition {
    ...
    public Text getDescription() {
        return switch (mode) {
            case ON_BLOCK -> Text.translatable("condition.lumenechallenge.position.on_block");
            case IN_AIR -> Text.translatable("condition.lumenechallenge.position.in_air");
            case STANDING_ON_BLOCK -> {
                var block = Registries.BLOCK.get(Identifier.of(blockId));
                yield Text.translatable("condition.lumenechallenge.position.on_specific_block", Text.translatable(block.getTranslationKey()));
            }
            case Y_RANGE -> Text.translatable("condition.lumenechallenge.position.y_range", a, b);
            case ABOVE_Y -> Text.translatable("condition.lumenechallenge.position.above_y", a);
            case BELOW_Y -> Text.translatable("condition.lumenechallenge.position.below_y", a);
            case EXACT_Y -> Text.translatable("condition.lumenechallenge.position.exact_y", a);
        };
    }
