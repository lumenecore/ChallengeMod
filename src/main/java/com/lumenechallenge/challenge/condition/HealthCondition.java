...
public final class HealthCondition extends AbstractCondition {
    ...
    public Text getDescription() {
        if (Math.abs(minHealth - maxHealth) < 0.001f) {
            return Text.translatable("condition.lumenechallenge.health.exact", hearts(minHealth));
        }
        return Text.translatable("condition.lumenechallenge.health.range", hearts(minHealth), hearts(maxHealth));
    }
