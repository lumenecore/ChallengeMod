...
public final class ChallengeNetworking {
    ...
    public static void registerServer() {
        ...
        PayloadTypeRegistry.playC2S().register(RerollChallengePayload.ID, RerollChallengePayload.CODEC);
        ...
        ServerPlayNetworking.registerGlobalReceiver(SelectSeedPayload.ID, (payload, context) -> {
            ...
            RunDifficulty difficulty = RunDifficulty.fromId(payload.difficultyId());
            int challengeCount = Math.max(1, Math.min(100, payload.challengeCount()));
            ...
            context.server().execute(() -> {
                ...
                state.selectSeed(player, seed, modifier, difficulty, challengeCount);
                state.setChallengeSeedText(payload.seed());
                state.markDirty();
            });
        });

        ServerPlayNetworking.registerGlobalReceiver(RerollChallengePayload.ID, (payload, context) ->
                context.server().execute(() -> {
                    ServerPlayerEntity player = context.player();
                    if (!WorldMarkerUtil.isMarked(player.getServer())) return;
                    ChallengeState state = ChallengeState.getServerState(player.getServer());
                    if (state.challenge() == null || state.completed()) {
                        return;
                    }
                    state.rerollCurrentTask(player);
                })
        );
    }
}
