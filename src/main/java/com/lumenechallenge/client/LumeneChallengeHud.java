    ...
    public static void render(DrawContext context, RenderTickCounter tickCounter) {
        ...
        if (!state.completed() && state.challenge() != null) {
            ...
            drawCenteredWrappedText(
                    context,
                    textRenderer,
                    Text.translatable("hud.lumenechallenge.progress", percent),
                    panelX + 6,
                    progressTop,
                    panelWidth - 12,
                    progressBlockHeight,
        ...
        if (state.timerStarted()) {
            ...
            int timerX = screenWidth - timerWidth - MARGIN;
            int timerY = MARGIN;
            ...
            if (state.modifierEnabled() && modifier.isVisible()) {
                int modifierX = timerX + timerWidth - GuiPanelRenderer.MODIFIER_ICON_SIZE;
                int modifierY = timerY + timerHeight + MODIFIER_GAP;
                ...
                String modifierLabel = Text.translatable(modifier.translationKey()).getString();
        ...
        if (state.seedSelected() && LumeneChallengeClient.isSeedVisible()) {
            Text seedText = Text.translatable("hud.lumenechallenge.seed", Long.toString(state.challengeSeed()));
            context.drawTextWithShadow(textRenderer, seedText, MARGIN, screenHeight - textRenderer.fontHeight - MARGIN, 0xFFFFFF);
        }
    }
